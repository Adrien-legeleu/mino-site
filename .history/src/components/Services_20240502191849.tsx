  ...
  return (
    <div className="h-full w-full bg-grayLight pt-48 pb-48 max-w-[1700px] m-auto">
      ...
      <div className=" flex flex-col lg:grid lg:grid-cols-30/70 mt-16">
        <div className="hidden bg-grayDark rounded-3xl lg:flex lg:flex-col items-center w-1/2 m-auto justify-center gap-10">
          {servicesInfos.map((serviceInfo, index) => (
            <div
              key={`serviceInfo : ${index}`}
              className="p-5 flex flex-col gap-5 justify-between"
            >
              <div className="bg-[rgba(30,30,30,0.4)] lg:rounded-3xl h-[50%] sm:h-[70%] xl:h-[60%] 2xl:h-[85%]  w-full flex items-center justify-center ">
                <img
                  src={serviceInfo.img}
                  alt={serviceInfo.title}
                  className={`w-[60%] object-contain relative ${
                    index == 1 ? "top-8" : ""
                  }`}
                />
              </div>
