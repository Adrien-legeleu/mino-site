        ...
        {window.innerWidth > 768 ? (
          <div className="flex flex-col gap-6">
            {Cards.map((card, index) => (
              <div
                key={index}
                className="grid grid-cols-30/70 bg-grayDark rounded-3xl h-[200px] sease-out duration-300 group relative"
              >
                <div className=" bg-[rgba(30,30,30,0.4)]   lg:rounded-3xl rounded-2xl h-[50%] sm:h-[70%] xl:h-[80%] 2xl:h-[95%] w-full flex items-center justify-center relative">
                  <img
                    className="duration-300 ease-out w-[50%] h-[100%] object-contain"
                    src={card.img}
                    alt={card.title}
                  />
                </div>
                <div className="text-whiteGray flex flex-col justify-between p-10  h-full z-10">
                  ...
                  <p className="text-sm xl:text  pr-20">{card.text}</p>
                </div>
              </div>
            ))}
          </div>
