          ...
          >
            {Cards.map((card, index) => (
              <SplideSlide>
                <div
                  key={index}
                  className="grid grid-cols-20/80 bg-grayDark rounded-3xl h-[200px] items-center sease-out duration-300 group relative"
                >
                  <div className=" bg-[rgba(30,30,30,0.4)]   lg:rounded-3xl rounded-2xl h-[50%]  sm:h-[70%] xl:h-[60%] 2xl:h-[80%] w-1/2 flex items-center justify-center relative m-auto">
                    <img
                      className="duration-300 ease-out w-[50%]  object-contain"
                  ...
                  <div className=" flex flex-col justify-between  h-full">
                    <h4 className="text-white text">{card.title}</h4>
                    <p className="text-whiteGray text-sm  pr-20">{card.text}</p>
                  </div>
                </div>
              </SplideSlide>
            ))}
