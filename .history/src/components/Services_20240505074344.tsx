...
const Services = () => {
  ...
  useEffect(() => {
    ...
    if (leftElement && servicesElement) {
      // Fonction pour gérer le défilement
      const handleScroll = () => {
        const triggerTop = servicesElement.getBoundingClientRect().top;
        const triggerBottom = servicesElement.getBoundingClientRect().bottom;
        const windowHeight = window.innerHeight;

        if (triggerTop < windowHeight && triggerBottom > 0) {
          // L'élément est dans la vue
          const scrollPercentage = (windowHeight - triggerTop) / windowHeight;
          const translateY = (1 - scrollPercentage) * leftElement.offsetHeight;
          gsap.to(leftElement, { y: translateY });
        }
      };
