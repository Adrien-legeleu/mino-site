    ...
    return(
        <div className="lg:grid lg:grid-cols-30/70 bg-grayLight pt-40 pl-8 pr-8 lg:pr-16 flex flex-col">
            ...
            <div className="flex flex-col gap-32">
                <p className="text-white  tracking-wide leading-[55px] indent-12 md:text-4xl sm:text-2xl  lg:text-3xl xl:text-5xl  mt-10 lg:m-0">
                ...
                <div  className="flex flex-col gap-4">
                    ...
                    Cards.map((card , index)=>(
                        <div key={index} className=" relative md:grid md:grid-cols-2 flex flex-col bg-grayDark rounded-3xl h-[220px] md:h-[350px] shadow-card  hover:brightness-125 ease-out duration-300 group">
                            <div className="text-whiteGray flex flex-col justify-between p-10  h-full">
                                <h4 className="sm:text-xl text-md md:text-2xl">{card.title}</h4>
                                <p className="text-sm sm:text-md lg:text-sm xl:text-xl">{card.text}</p>
                            </div>
                            <div className="md:w-full md:h-full flex items-center justify-center absolute w-[100px] h-[100px] top-3 right-4 md:relative">
                                <img className="group-hover:scale-110 duration-300 ease-out w-[95%] h-[95%] object-contain" src={card.img} alt={card.title} />
                            </div>
                        </div>
                    ))
                }
