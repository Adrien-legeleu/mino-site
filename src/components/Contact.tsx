...
const Contact = () => {
  ...
  return (
    <div className="bg-grayLight shadow-contact_shadow pt-48 flex flex-col lg:grid lg:grid-cols-30/70 pl-10 pr-10 pb-32" id="contact">
      ...
      <div>
        ...
        {contactText.map((desc, index) => (
          <div
            className="grid grid-cols-90/10 md:p-10 p-5 pt-10 pb-10 h-48 w-full bg-grayDark justify-center rounded-3xl group relative mb-4"
            key={index}
          >
            ...
            <div className="grid grid-cols-20/80 gap-8 group-hover:translate-x-5 duration-500 ease-in-out z-10 items-center">
              <div className="bg-[rgba(30,30,30,0.4)] rounded-3xl h-2/3 w-full flex items-center justify-center ">
                <img
                  className="w-2/3"
                  src={desc.icon}
                  alt={`icon : ${index}`}
